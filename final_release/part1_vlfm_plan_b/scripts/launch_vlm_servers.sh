...
export GROUNDING_DINO_CUDA_DEVICE=${GROUNDING_DINO_CUDA_DEVICE:-0}
export BLIP2ITM_CUDA_DEVICE=${BLIP2ITM_CUDA_DEVICE:-1}
export SAM_CUDA_DEVICE=${SAM_CUDA_DEVICE:-2}
export YOLOV7_CUDA_DEVICE=${YOLOV7_CUDA_DEVICE:-3}
...
# Run commands in each pane. The device bindings are configurable so this script still
# works on machines with different GPU layouts.
tmux send-keys -t ${session_name}:0.0 "CUDA_VISIBLE_DEVICES=${GROUNDING_DINO_CUDA_DEVICE} ${VLFM_PYTHON} -m vlfm.vlm.grounding_dino --port ${GROUNDING_DINO_PORT}" C-m
tmux send-keys -t ${session_name}:0.1 "CUDA_VISIBLE_DEVICES=${BLIP2ITM_CUDA_DEVICE} ${VLFM_PYTHON} -m vlfm.vlm.blip2itm --port ${BLIP2ITM_PORT}" C-m
tmux send-keys -t ${session_name}:0.2 "CUDA_VISIBLE_DEVICES=${SAM_CUDA_DEVICE} ${VLFM_PYTHON} -m vlfm.vlm.sam --port ${SAM_PORT}" C-m
tmux send-keys -t ${session_name}:0.3 "CUDA_VISIBLE_DEVICES=${YOLOV7_CUDA_DEVICE} ${VLFM_PYTHON} -m vlfm.vlm.yolov7 --port ${YOLOV7_PORT}" C-m
