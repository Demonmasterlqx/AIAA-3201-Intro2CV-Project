...
def build_episode_stats(
    ...
    failure_cause = "did_not_fail" if infos["success"] == 1 else determine_failure_cause(infos)
    failure_reason = determine_failure_reason(infos, extra)
    infos_no_map = infos.copy()
    infos_no_map.pop("top_down_map", None)
    merged = {
...
def determine_failure_reason(infos: Dict[str, Any], extra_data: Optional[Mapping[str, Any]] = None) -> str:
    extra = extra_data or {}
    if infos["success"] == 1:
        return "success"

    termination_hint = str(extra.get("termination_hint", "")).strip()
    if termination_hint in {
        "frontier_collapse",
        "mapping_issue",
        "peer_success_abort",
        "no_candidate_goals",
    }:
        return termination_hint

    if bool(extra.get("stuck_low_progress", False)):
        return "stuck_low_progress"

    if infos.get("target_detected", False):
        if was_false_positive(infos):
            return "false_positive"
        return "timeout_after_detection"

    return "target_not_found_timeout"
