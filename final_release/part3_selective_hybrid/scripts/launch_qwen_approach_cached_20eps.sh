...
SESSION=qwenvl_objectpatch_approachcache20
RESULT_DIR=results/baseline_compare_single/qwenvl_objectpatch_qwen_approach_cached_20eps_v1
...
case "$RESULT_DIR" in
  results/baseline_compare_single/qwenvl_objectpatch_qwen_approach_cached_20eps_v1)
    rm -rf "$RESULT_DIR"
    ;;
  *)
...
python -u -m ascent.run \
  ...
  habitat.environment.iterator_options.num_episode_sample=20 \
