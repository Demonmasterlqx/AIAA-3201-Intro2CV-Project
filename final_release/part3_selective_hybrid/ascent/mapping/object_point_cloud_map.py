...
class ObjectPointCloudMap(BaseMap):
    ...
    def has_object(self, target_class: str) -> bool:
        return (
            (target_class in self.clouds and len(self.clouds[target_class]) > 0)
            or (target_class in self.verified_clouds and len(self.verified_clouds[target_class]) > 0)
        )
    ...
    def extract_global_cloud(
        ...
        if len(local_cloud) == 0:
            return np.empty((0, 4))
        if compact:
            raw_count = len(local_cloud)
            local_cloud = self._compact_verified_local_cloud(local_cloud)
            if len(local_cloud) == 0:
                return np.empty((0, 4))
            if len(local_cloud) != raw_count:
                print(f"Qwen-VL verified cloud compacted from {raw_count} to {len(local_cloud)} local points.")
        within_range = (local_cloud[:, 0] <= max_depth * 0.95).astype(np.float32)
        within_range[within_range == 0] = np.random.rand()
        global_cloud = transform_points(tf_camera_to_episodic, local_cloud)
        global_cloud = np.concatenate((global_cloud, within_range[:, None]), axis=1)
        pixel_points = self._xy_to_px(global_cloud[:, :2])
        valid_points_mask = ~self._disabled_object_map[pixel_points[:, 1], pixel_points[:, 0]]
        return global_cloud[valid_points_mask]

    def set_verified_cloud(self, object_name: str, global_cloud: np.ndarray) -> None:
        if len(global_cloud) == 0:
            return
        if object_name in self.verified_clouds:
            self.verified_clouds[object_name] = np.concatenate(
                (self.verified_clouds[object_name], global_cloud),
                axis=0,
            )
        else:
            self.verified_clouds[object_name] = global_cloud
    ...
    @staticmethod
    def cloud_quality_stats(global_cloud: np.ndarray) -> Dict[str, float]:
        if len(global_cloud) == 0:
            return {
                "num_points": 0.0,
                "median_depth": 0.0,
        ...
        xy_radius = np.linalg.norm(xyz[:, :2] - center[:2], axis=1)
        z_abs = np.abs(xyz[:, 2] - center[2])
        depth = np.linalg.norm(xyz, axis=1)
        return {
            "num_points": float(len(global_cloud)),
            "median_depth": float(np.median(depth)),
            "xy_radius_p75": float(np.percentile(xy_radius, 75)),
            "xy_radius_p90": float(np.percentile(xy_radius, 90)),
            "z_span_p90": float(np.percentile(z_abs, 90) * 2.0),
        }

    @staticmethod
    def _compact_verified_local_cloud(local_cloud: np.ndarray) -> np.ndarray:
        if len(local_cloud) < 80:
            return local_cloud

        xyz = local_cloud[:, :3]
        median = np.median(xyz, axis=0)
        depth_delta = np.abs(xyz[:, 0] - median[0])
        lateral_radius = np.linalg.norm(xyz[:, 1:3] - median[1:3], axis=1)

        # Qwen has already confirmed the cropped patch visually. This trim only
        # removes depth/mask spillover that would otherwise pull the navigation
        # target to a wall or floor behind the object.
        depth_keep = depth_delta <= max(0.35, float(np.percentile(depth_delta, 75)))
        lateral_keep = lateral_radius <= max(0.45, float(np.percentile(lateral_radius, 80)))
        keep = depth_keep & lateral_keep
        compact_cloud = local_cloud[keep]

        if len(compact_cloud) < 80 or len(compact_cloud) < len(local_cloud) * 0.08:
            return local_cloud
        return compact_cloud

    def clear_verified_cloud(self, object_name: str) -> None:
        if object_name in self.verified_clouds:
            del self.verified_clouds[object_name]
        self.last_target_coord = None
    ...
    def get_target_cloud(self, target_class: str) -> np.ndarray:
        source_clouds = self.verified_clouds if target_class in self.verified_clouds else self.clouds
        target_cloud = source_clouds[target_class].copy()
