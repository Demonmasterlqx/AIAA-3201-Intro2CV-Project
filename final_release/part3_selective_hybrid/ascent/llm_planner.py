...
class Ascent_LLM_Planner:
    ...
    def get_logging_info(self) -> Dict[str, Any]:
        return self._llm.get_logging_info()

    @staticmethod
    def _extract_json_object(response: str) -> Dict[str, Any]:
        """Return the last valid JSON object embedded in an LLM response."""
    ...
    def _get_best_frontier_with_llm(
            ...
            # 5. 处理前沿点粘滞/循环检测和禁用
            # 这一部分逻辑相对独立且复杂，可以封装
            best_frontier, best_value = self._sanitize_frontier_choice(best_frontier, best_value, sorted_pts, sorted_values)
            if best_frontier is None:
                print("No valid frontier available after LLM decision and fallback.")
                return np.array([]), 0.0
    ...
    def _sanitize_frontier_choice(self, best_frontier, best_value, sorted_pts, sorted_values):
        """Keep malformed local-Qwen frontier choices from aborting long evals."""
        try:
            frontier = np.asarray(best_frontier, dtype=np.float64)
        except (TypeError, ValueError):
            frontier = np.array([])
        if frontier.size >= 2 and np.all(np.isfinite(frontier[:2])):
            return frontier[:2], best_value
        if len(sorted_pts) == 0:
            return None, best_value
        fallback_frontier = np.asarray(sorted_pts[0], dtype=np.float64)[:2]
        fallback_value = sorted_values[0] if len(sorted_values) > 0 else 0.0
        print(
            "Invalid or empty LLM frontier choice; falling back to top value frontier "
            f"{fallback_frontier}."
        )
        return fallback_frontier, fallback_value
    ...
    def _sort_frontiers_by_value(
        ...
        for pt, val in zip(raw_sorted_pts, raw_sorted_values):
            if not obstacle_map[env].is_frontier_disabled(pt):
    ...
    def _handle_frontier_stick_and_disable(self, current_best_frontier, robot_xy, env, last_frontier_distance, frontier_stick_step, obstacle_map,):
        # 将前沿点粘滞和禁用逻辑封装
        try:
            current_best_frontier = np.asarray(current_best_frontier, dtype=np.float64)
        except (TypeError, ValueError):
            frontier_stick_step[env] = 0
            last_frontier_distance[env] = 0
            return
        if current_best_frontier.size < 2 or not np.all(np.isfinite(current_best_frontier[:2])):
            frontier_stick_step[env] = 0
            last_frontier_distance[env] = 0
            return
        current_best_frontier = current_best_frontier[:2]
    ...
    def llm_analyze_single_floor(self, env, target_object_category, frontier_index_list, obstacle_map, object_map):
        ...
        """
        def choose_valid_index(raw_index: Any, raw_response: str = "") -> int:
            candidates: List[int] = []
            for value in (raw_index, raw_response):
                if value is None:
                    continue
                candidates.extend(int(match) for match in re.findall(r"-?\d+", str(value)))
            for candidate in candidates:
                if 1 <= candidate <= len(frontier_index_list):
                    return candidate - 1
            if candidates:
                clamped = min(max(candidates[0], 1), len(frontier_index_list))
                logging.warning(
                    "LLM frontier index %s is out of range 1..%s; clamping to %s.",
                    candidates[0],
                    len(frontier_index_list),
                    clamped,
                )
                return clamped - 1
            logging.warning("No valid frontier index found in LLM response; falling back to top frontier.")
            return 0
        ...
        if response == "-1":
            ...
            response_dict = self._extract_json_object(response)
            if not response_dict:
                logging.warning(f"Failed to parse JSON response: {response}")
                temp_frontier_index = choose_valid_index(None, response)
            else:
                # Extract Index
                ...
                if index == "N/A":
                    ...
                    temp_frontier_index = choose_valid_index(index, response)
    ...
    def _extract_multiple_floor_decision(self, multi_floor_response, env, cur_floor_index) -> int:
        ...
        try:
            # 解析 LLM 的回复
            response_dict = self._extract_json_object(multi_floor_response)
            if not response_dict:
                raise json.JSONDecodeError("No JSON object found", multi_floor_response, 0)
