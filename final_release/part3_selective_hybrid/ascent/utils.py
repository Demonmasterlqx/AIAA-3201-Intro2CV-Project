...
from ascent.llm_config import (
    FrontierRecoveryConfig,
    LLMBackendConfig,
    StairBudgetConfig,
    VisionLLMBackendConfig,
)
cv2 = try_cv2_import()
from typing import (
    TYPE_CHECKING,
...
def load_floor_probabilities_by_dataset(dataset_type: str):
    ...
    elif dataset_type == "hssd-hab":
        return None
    else:
        raise ValueError(f"Dataset type {dataset_type} not recognized")    


def infer_dataset_type_from_path(data_path: str) -> str:
    """Infer the dataset type from the configured Habitat dataset path."""
    normalized_path = data_path.lower()
    if "hssd-hab" in normalized_path:
        return "hssd-hab"
    if "hm3d" in normalized_path:
        return "hm3d"
    if "mp3d" in normalized_path:
        return "mp3d"
    raise ValueError(f"Dataset type could not be inferred from habitat config path: {data_path}")


def _goal_name_from_dataset_id(dataset_type: str, object_id: int) -> str:
    """Resolve a dataset-specific goal id to a readable object category name."""
    if dataset_type == "hm3d":
        return HM3D_ID_TO_NAME[object_id]
    if dataset_type == "mp3d":
        return MP3D_ID_TO_NAME[object_id]
    return str(object_id)


def resolve_object_goal_names(
    dataset_type: str, object_ids: Any, current_episodes_info: Optional[List[Any]] = None
) -> List[str]:
    """Resolve readable goal names for each environment.

    Preference order:
    1. The current episode's ``object_category`` when it is available.
    2. The historic HM3D/MP3D hard-coded mappings.
    3. The raw goal id as a string.
    """
    goal_names: List[str] = []
    for env_index, object_id in enumerate(object_ids):
        episode = None
        if current_episodes_info is not None and env_index < len(current_episodes_info):
            episode = current_episodes_info[env_index]
        object_category = getattr(episode, "object_category", None)
        if object_category:
            goal_names.append(str(object_category))
        else:
            goal_names.append(_goal_name_from_dataset_id(dataset_type, object_id.item()))
    return goal_names


def build_non_coco_caption(dataset_type: str, goal_names: List[str]) -> str:
    """Build the GroundingDINO caption used for non-COCO detections."""
    if dataset_type == "mp3d":
        return " . ".join(MP3D_ID_TO_NAME).replace("|", " . ") + " ."

    caption_terms: List[str] = []
    for goal_name in goal_names:
        for candidate in str(goal_name).split("|"):
            cleaned = candidate.strip()
            if cleaned and cleaned not in caption_terms:
                caption_terms.append(cleaned)

    if not caption_terms:
        return ""
    return " . ".join(caption_terms) + " ."
...
class AscentPolicyConfig(VLFMPolicyConfig):
    ...
    final_check: VisionLLMBackendConfig = field(default_factory=VisionLLMBackendConfig)
    frontier: FrontierRecoveryConfig = field(default_factory=FrontierRecoveryConfig)
    stair: StairBudgetConfig = field(default_factory=StairBudgetConfig)
...
class MultiFloorTopDownMap(FrontierExplorationMap):
    ...
    def _safe_draw_map_overlay(self, draw_fn, episode: NavigationEpisode, *args: Any) -> None:
        """Keep optional top-down-map overlays from aborting evaluation."""
        try:
            draw_fn(episode, *args)
        except Exception as exc:
            print(
                f"[WARN] Skipping {getattr(draw_fn, '__name__', str(draw_fn))} "
                f"for episode {getattr(episode, 'episode_id', 'unknown')}: {exc}"
            )
    ...
    def reset_metric(
        ...
        if hasattr(episode, "goals"):
            ...
            self._safe_draw_map_overlay(self._draw_goals_view_points, episode)
            self._safe_draw_map_overlay(self._draw_goals_aabb, episode)
            self._safe_draw_map_overlay(self._draw_goals_positions, episode)
            self._safe_draw_map_overlay(self._draw_shortest_path, episode, agent_position)
        ...
        self._safe_draw_map_overlay(self._draw_target_bbox_mask, episode)
        self._static_metrics.setdefault(
            "target_bboxes_mask",
