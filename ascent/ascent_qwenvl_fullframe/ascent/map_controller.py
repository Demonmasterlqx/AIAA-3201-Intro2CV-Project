...
class Map_Controller:
    ...
    def __init__(self, device, num_envs: int, map_size: int = 1600, text_prompt: str = "", object_map_erosion_size: int = 1600,
        ...
        self._final_check_mode: List[str] = [
            "qwenvl_fullframe" if self._final_check_cfg is not None else "disabled"
        ] * self._num_envs
    ...
    def _build_final_check_prompt(self, env: int) -> str:
        ...
        return (
            "You are verifying whether the target object is clearly visible in the image. "
            f'Target object: "{target_name}". '
            "Respond with JSON only using this exact schema: "
            '{"is_target_present": true, "confidence": 0.0, "reason": ""}. '
            "Only answer true if the target object itself is visible in the image. "
            "Do not count nearby furniture, similar objects, or likely context. "
    ...
    @staticmethod
    def _resolve_target_present_flag(payload: Dict[str, Any]) -> bool:
        if "is_target_present" in payload:
            return bool(payload["is_target_present"])
        for key, value in payload.items():
            lowered = str(key).lower()
            if "target_present" in lowered or (lowered.endswith("present") and "target" in lowered):
                return bool(value)
        return False

    def _run_final_check_fullframe(self, env: int, rgb: np.ndarray, step: int) -> bool:
        ...
        raw_response = self._final_check_client.chat_image(rgb, self._build_final_check_prompt(env))
        ...
        target_present = self._resolve_target_present_flag(payload)
    ...
    def _update_object_map_with_stair_and_person(self, height: int, width: int, observations_cache: List[Dict], non_coco_caption: str, num_steps: List[int], try_to_navigate: List[bool]):
        ...
        for env in range(self._num_envs):
            ...
            # --- 处理目标对象 ---
            has_target_detection = len(target_detections.logits) > 0
            ...
            if has_target_detection and try_to_navigate[env] and not self._double_check_goal[env]:
                final_check_ok = self._run_final_check_fullframe(env, rgb, num_steps[env])
                print(
                    f"Qwen-VL final check confidence: {self._final_check_confidence[env]:.3f}, "
                    f"decision={final_check_ok}"
                )
                if final_check_ok:
                    self._double_check_goal[env] = True
                    print("Qwen-VL double check success!!!")
