...
def send_request(url: str, **kwargs: Any) -> dict:
    ...
    for attempt in range(10):
        try:
            ...
            if attempt == 9:
                raise RuntimeError(f"{endpoint} request failed after 10 attempts: {e}") from e
            else:
                print(f"Error: {endpoint} -> {e}. Retrying in 20-30 seconds...")
...
def _send_request(url: str, **kwargs: Any) -> dict:
    ...
    try:
        ...
        request_timeout = float(os.environ.get("MODEL_HTTP_TIMEOUT_SHORT", "1"))
        deadline_seconds = float(os.environ.get("MODEL_HTTP_DEADLINE_SHORT", "20"))
        ...
        while True:
            try:
                ...
                if time.time() - start_time > deadline_seconds:
                    raise Exception(f"Request timed out after {deadline_seconds} seconds")
...
def send_request_vlm(url: str, timeout: int = 20, **kwargs: Any) -> dict:
    ...
    for attempt in range(10):
        try:
            ...
            if attempt == 2:
                error_message = f"{endpoint} request failed after 3 attempts: {e}"
                raise Exception(error_message) from e
            else:
                print(f"Error: {endpoint} -> {e}. Retrying in 5-10 seconds...")
