...
def load_pointnav_policy(
    ...
    if HABITAT_BASELINES_AVAILABLE:
        ...
        if habitat_version == "0.1.5":
            ...
            if agent_name:
                config_for_policy = copy.deepcopy(config_to_use)
                with read_write(config_for_policy.habitat_baselines.rl.policy):
                    with open_dict(config_for_policy.habitat_baselines.rl.policy):
                        if agent_name not in config_for_policy.habitat_baselines.rl.policy:
                            config_for_policy.habitat_baselines.rl.policy[agent_name] = (
                                config_for_policy.habitat_baselines.rl.policy.main_agent
                            )
