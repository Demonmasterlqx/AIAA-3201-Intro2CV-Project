...
class Ascent_Policy(HabitatMixin, ITMPolicyV2):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        pointnav_agent_name = (
            ...
            if config and len(config.habitat.simulator.agents_order) > 1
            else "main_agent"
        )
        self._pointnav_policy: List[WrappedPointNavResNetPolicy] = [
            WrappedPointNavResNetPolicy(
                ...
                agent_name=pointnav_agent_name,
        ...
        self._region_target_override: List[Optional[np.ndarray]] = [None] * self._num_envs
        self._region_target_radius: float = 2.5
    ...
    def _reset(self, env: int) -> None:
        ...
        self._region_target_override[env] = None
    ...
    def _explore(self, observations: Union[Dict[str, Tensor], "TensorDict"], env: int, masks: Tensor) -> Tensor:
        ...
        if self._region_target_override[env] is not None and len(frontiers) > 0:
            target_centroid = self._region_target_override[env]
            filtered_frontiers = [
                f
                for f in frontiers
                if float(np.linalg.norm(np.asarray(f)[:2] - target_centroid[:2])) <= self._region_target_radius
            ]
