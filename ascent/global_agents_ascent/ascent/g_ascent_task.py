...
@registry.register_task(name="GlobalMultiAgentObjectNav-v0")
class GlobalMultiAgentObjectNavigationTask(ObjectNavigationTask):
...
@registry.register_sensor(name="GAgentHeadingSensor")
class GAgentHeadingSensor(HeadingSensor):
...
@registry.register_sensor(name="GAgentCompassSensor")
class GAgentCompassSensor(GAgentHeadingSensor):
...
@registry.register_sensor(name="GAgentGPSSensor")
class GAgentGPSSensor(EpisodicGPSSensor):
    def __init__(self, sim, config, *args: Any, **kwargs: Any):
        self.agent_id = getattr(config, "agent_id", 0)
        super().__init__(sim, config, *args, **kwargs)
...
@registry.register_sensor(name="GAgentObjectGoalSensor")
class GAgentObjectGoalSensor(ObjectGoalSensor):
...
class GlobalJointNavAction(SimulatorTaskAction):
...
class GlobalJointSPL(Measure):
    ...
    def reset_metric(self, episode, task, *args: Any, **kwargs: Any) -> None:
        task.measurements.check_measure_dependencies(
            self.uuid, [GlobalJointDistanceToGoal.cls_uuid, GlobalJointSuccess.cls_uuid]
        )
        ...
        self._start_end_episode_distance = task.measurements.measures[
            GlobalJointDistanceToGoal.cls_uuid
        ].get_metric()
        self.update_metric(episode=episode, task=task, *args, **kwargs)

    def update_metric(self, episode, task, *args: Any, **kwargs: Any) -> None:
        assert self._previous_positions is not None
        ep_success = float(task.measurements.measures[GlobalJointSuccess.cls_uuid].get_metric())
        total_delta = 0.0
        for agent_id in range(len(self._sim.habitat_config.agents_order)):
...
@registry.register_measure
class GlobalJointDistanceToGoal(Measure):
...
@registry.register_measure
class GlobalJointSuccess(Measure):
    ...
    def reset_metric(self, episode, task, *args: Any, **kwargs: Any) -> None:
        task.measurements.check_measure_dependencies(
            self.uuid, [GlobalJointDistanceToGoal.cls_uuid]
        )
        self.update_metric(episode=episode, task=task, *args, **kwargs)
...
class AgentObjectGoalSensorCfg(ObjectGoalSensorConfig):
    type: str = "GAgentObjectGoalSensor"
