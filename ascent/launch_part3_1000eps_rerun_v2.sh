...
ENV_FILE="$ASSET_REPO/.env"
SESSION=part3_1000eps_rerun_v2
SINGLE_RESULT="results/baseline_compare_single/20260502_single_1000eps_rerun_v2"
OBJ_RESULT="results/baseline_compare_single/qwenvl_objectpatch_weakcache_1000eps_rerun_v2"
...
cat > "$BASE/part3_1000eps_queue_v2.sh" <<QUEUEEOF
#!/usr/bin/env bash
set -u pipefail
source /data/home/sim6g/anaconda3/etc/profile.d/conda.sh
conda activate ascent_nav
...
QUEUE_LOG="$BASE/part3_1000eps_queue_v2.log"

printf 'PART3 1000eps queue v2 started at %s\n' "\$(date -Is)" | tee "\$QUEUE_LOG"
printf 'SINGLE_REPO=%s\nOBJ_REPO=%s\n' "\$SINGLE_REPO" "\$OBJ_REPO" | tee -a "\$QUEUE_LOG"
...
printf '[2/2] Qwen-VL objectpatch weakcache finished with %s at %s\n' "\$obj_code" "\$(date -Is)" | tee -a "\$QUEUE_LOG"
exit "\$obj_code"
QUEUEEOF
chmod +x "$BASE/part3_1000eps_queue_v2.sh"

tmux new-session -d -s "$SESSION" "bash '$BASE/part3_1000eps_queue_v2.sh'"
echo "started $SESSION"
echo "single_log=$SINGLE_REPO/$SINGLE_RESULT/run.log"
echo "objectpatch_log=$OBJ_REPO/$OBJ_RESULT/run.log"
sleep 5
tmux has-session -t "$SESSION"
tail -n 80 "$BASE/part3_1000eps_queue_v2.log" || true
