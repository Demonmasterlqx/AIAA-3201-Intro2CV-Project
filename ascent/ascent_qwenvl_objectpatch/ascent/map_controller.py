...
class Map_Controller:
    ...
    def __init__(self, device, num_envs: int, map_size: int = 1600, text_prompt: str = "", object_map_erosion_size: int = 1600,
        ...
        if self._final_check_cfg is not None:
            try:
        ...
        self._final_check_mode: List[str] = [
            "qwenvl_objectpatch" if self._final_check_cfg is not None else "disabled"
        ] * self._num_envs
    ...
    def _build_final_check_prompt(self, env: int) -> str:
        ...
        alias_hint = target_aliases.get(target_name.lower(), target_name)
        visual_target_name = "television (TV set/display screen)" if target_name.lower() == "tv" else target_name
        return (
    ...
    def _maybe_override_tv_false_negative(
        ...
        if target_present or target_name.lower() != "tv":
    ...
    def _run_final_check_image(self, env: int, image: np.ndarray, step: int, mode: str) -> bool:
        ...
        decision = target_present and confidence >= float(
            getattr(self._final_check_cfg, "confidence_threshold", 0.70)
        )
        self._final_check_confidence[env] = confidence
    ...
    def _extract_object_patch(self, rgb: np.ndarray, mask: np.ndarray, bbox_denorm: np.ndarray) -> Tuple[Optional[np.ndarray], Optional[List[int]]]:
        ...
        # Keep detector context instead of a mask-tight crop; Qwen-VL needs a little
    ...
    def _lookup_patch_cache(self, env: int, patch_bbox: List[int], step: int) -> Optional[Dict[str, Any]]:
        for entry in self._final_check_patch_cache[env]:
            if self._bbox_iou(entry["bbox"], patch_bbox) >= 0.7 and step - entry["step"] <= 5:
                return entry
        return None
    ...
    def _count_recent_positive_patches(self, env: int, patch_bbox: List[int], step: int) -> int:
    ...
    def _has_recent_positive_patch(self, env: int, patch_bbox: List[int], step: int) -> bool:
        return self._count_recent_positive_patches(env, patch_bbox, step) > 0

    def _has_enough_weak_positive_patches(self, env: int, patch_bbox: List[int], step: int) -> bool:
        required = int(getattr(self._final_check_cfg, "weak_confirmation_min_supports", 3))
        return self._count_recent_positive_patches(env, patch_bbox, step) >= max(1, required - 1)
    ...
    def _run_final_check_objectpatch(
        ...
        for rank, bbox_denorm, target_object_mask in target_patch_candidates[:3]:
            ...
            if cached is not None:
                ...
                strong_threshold = float(getattr(self._final_check_cfg, "strong_confidence_threshold", 0.92))
                ...
                confirmation_threshold = float(
                    getattr(self._final_check_cfg, "confirmation_confidence_threshold", 0.90)
                )
                ...
                weak_threshold = float(
                    getattr(self._final_check_cfg, "weak_confirmation_confidence_threshold", 0.90)
                )
                if cached_confidence >= weak_threshold and self._has_enough_weak_positive_patches(env, patch_bbox, step):
                    self._final_check_pending_verified_cloud[env] = self._object_map[env].extract_global_cloud(
                        depth,
                        target_object_mask,
                        tf_camera_to_episodic,
                    ...
                    print("Qwen-VL patch cached weak multi-view confirmation accepted.")
                    self._remember_positive_patch(env, patch_bbox, step)
                    return True
            ...
            if decision:
                ...
                if (
                    self._final_check_confidence[env] >= confirmation_threshold
                    and self._has_recent_positive_patch(env, patch_bbox, step)
                ):
                ...
                if self._final_check_confidence[env] >= confirmation_threshold:
                    ...
                    if (
                        self._final_check_confidence[env] >= weak_threshold
                        and self._has_enough_weak_positive_patches(env, patch_bbox, step)
                    ):
                        print("Qwen-VL patch weak multi-view confirmation accepted.")
                        self._remember_positive_patch(env, patch_bbox, step)
                        return True
                    if self._final_check_confidence[env] >= weak_threshold:
                        print("Qwen-VL patch weak positive held for additional support.")
                        self._remember_positive_patch(env, patch_bbox, step)
                    else:
    ...
    def _update_object_map_with_stair_and_person(self, height: int, width: int, observations_cache: List[Dict], non_coco_caption: str, num_steps: List[int], try_to_navigate: List[bool]):
        ...
        for env in range(self._num_envs):
            ...
            if target_patch_candidates and try_to_navigate[env] and not self._double_check_goal[env]:
                final_check_ok = self._run_final_check_objectpatch(
                    env,
                    rgb,
                ...
                print(
                    f"Qwen-VL patch final check confidence: {self._final_check_confidence[env]:.3f}, "
                    f"decision={final_check_ok}, patches={self._final_check_patch_count[env]}"
                )
                if final_check_ok:
                    if self._final_check_pending_verified_cloud[env] is not None:
                        ...
                        print(
                            "Qwen-VL patch verified cloud locked "
                    ...
                    print("Qwen-VL patch double check success!!!")
