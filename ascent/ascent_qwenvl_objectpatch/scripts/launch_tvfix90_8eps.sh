...
SESSION=qwenvl_objectpatch_tvfix90_8
RESULT_DIR=results/baseline_compare_single/qwenvl_objectpatch_tv_reason_confirm90_8eps_v1
...
case "$RESULT_DIR" in
  results/baseline_compare_single/qwenvl_objectpatch_tv_reason_confirm90_8eps_v1)
    rm -rf "$RESULT_DIR"
    ;;
  *)
...
git diff HEAD~2..HEAD --stat > "$RESULT_DIR/diff_stat.txt"

cat > "$RESULT_DIR/run_command.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
source /data/home/sim6g/anaconda3/etc/profile.d/conda.sh
conda activate ascent_nav
...
tail -n 60 "$RESULT_DIR/run.log" || true
