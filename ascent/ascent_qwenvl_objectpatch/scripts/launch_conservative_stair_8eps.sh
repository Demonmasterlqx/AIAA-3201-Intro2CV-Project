...
SESSION=qwenvl_objectpatch_cons_stair8
RESULT_DIR=results/baseline_compare_single/qwenvl_objectpatch_conservative_stair_8eps_v1
...
case "$RESULT_DIR" in
  results/baseline_compare_single/qwenvl_objectpatch_conservative_stair_8eps_v1)
    rm -rf "$RESULT_DIR"
    ;;
  *)
...
git diff HEAD~1..HEAD --stat > "$RESULT_DIR/diff_stat.txt"
...
tail -n 40 "$RESULT_DIR/run.log" || true
