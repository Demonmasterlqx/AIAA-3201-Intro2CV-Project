...
class ObstacleMap(BaseMap):
    ...
    def __init__(
        ...
        self.disabled_frontier_radius = 1.0
    ...
    def is_frontier_disabled(self, frontier: np.ndarray, radius: float = None) -> bool:
        """Return True when a frontier falls inside a disabled frontier cluster."""
        if frontier is None or len(frontier) == 0 or len(self._disabled_frontiers) == 0:
            return False
        frontier_xy = np.asarray(frontier, dtype=np.float64)[:2]
        if tuple(frontier_xy) in self._disabled_frontiers:
            return True
        disabled = np.asarray(list(self._disabled_frontiers), dtype=np.float64)
        if disabled.ndim == 1:
            disabled = disabled.reshape(1, -1)
        if disabled.size == 0:
            return False
        check_radius = self.disabled_frontier_radius if radius is None else radius
        return bool(np.any(np.linalg.norm(disabled[:, :2] - frontier_xy, axis=1) <= check_radius))
