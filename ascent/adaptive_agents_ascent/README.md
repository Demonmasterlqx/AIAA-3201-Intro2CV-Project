...
### Optional: Local Qwen2.5-7B Weights
The default LLM backend is now API-based. Local Qwen deployment is optional.

If you want to use `local_qwen`, download the checkpoints through [HuggingFace](https://huggingface.co/Qwen/Qwen2.5-7B-Instruct) or [ModelScope](https://modelscope.cn/models/Qwen/Qwen2.5-7B-Instruct) and put them in `pretrained_weights/`
...
└── Qwen2.5-7b  # optional, only needed when llm.backend=local_qwen
...
- `data/scene_datasets/hssd-hab` can point to `data/versioned_data/hssd-hab`.
- `HSSD-Hab` scene assets are usable by Habitat-Lab, but ASCENT still requires `data/datasets/objectnav/hssd-hab/...` episode data before evaluation.

To check whether the local repository data layout is evaluation-ready, run:

```bash
python scripts/check_ascent_data.py --dataset all
```

For a more detailed mapping of `data/` directories to ASCENT usage, see [`docs/data_layout.md`](docs/data_layout.md).

## 🚀 Evaluation

### Default LLM Backend: DeepSeek API

Set the API credentials in `.env` or your shell environment:
...
### Launch Visual Model Servers

The launch script now starts only the visual model services by default. It does not start a local Qwen server unless explicitly requested.
...
If you want to use a locally deployed Qwen backend instead of DeepSeek API:

```bash
START_QWEN_SERVER=1 ./scripts/launch_vlm_servers_ascent.sh
```
...
Hydra backend switching examples:
