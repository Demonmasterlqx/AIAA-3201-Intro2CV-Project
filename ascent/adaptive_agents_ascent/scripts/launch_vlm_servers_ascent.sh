...
export START_QWEN_SERVER=${START_QWEN_SERVER:-0}
...
if [ "${START_QWEN_SERVER}" = "1" ]; then
  ...
  tmux send-keys -t ${session_name}:0.0 "echo 'Skipping local Qwen server. Configure DeepSeek via .env or set START_QWEN_SERVER=1 to enable local_qwen.'" C-m
fi
...
echo "Created tmux session '${session_name}'. You must wait up to 90 seconds for the visual model weights to finish being loaded."
echo "Local Qwen server enabled: ${START_QWEN_SERVER}"
