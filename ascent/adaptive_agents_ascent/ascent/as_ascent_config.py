...
class SpawnConfig:
    ...
    require_disjoint_frontier: bool = True
    allow_verifier_without_search: bool = True
    allow_verifier_priority_spawn: bool = True
...
class EvidenceFusionConfig:
    ...
    min_viewpoint_clusters: int = 2
    preferred_supporting_agents: int = 2


@dataclass
class SmokeEvalConfig:
    enabled: bool = True
    default_episode_count: int = 8
    dataset_split: str = "val"
    run_dir_template: str = "results/as_ascent/hm3d/{run_id}"


@dataclass
class LoggingConfig:
    log_dir_template: str = "results/as_ascent/hm3d/{run_id}"
    save_episode_jsonl: bool = True
    save_summary_jsonl: bool = True
    save_shared_memory_snapshots: bool = True
    record_assignment_rejections: bool = True
