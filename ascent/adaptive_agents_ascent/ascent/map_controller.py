...
class Map_Controller:
    ...
    def __init__(self, device, num_envs: int, map_size: int = 1600, text_prompt: str = "", object_map_erosion_size: int = 1600,
        ...
        self.PASSIVE_STAIR_COOLDOWN_STEPS = 25
        self.MIN_STAIR_PROGRESS_METERS = 0.75
        self.STAIR_POST_CENTROID_LIMIT = 20
        self._stair_post_centroid_steps: List[int] = [0] * self._num_envs
        self._passive_stair_cooldown: List[int] = [0] * self._num_envs
        self._stair_event_buffer: List[List[Dict[str, Any]]] = [[] for _ in range(self._num_envs)]
    def reset(self, env: int) -> None:
        ...
        self._stair_post_centroid_steps[env] = 0
        self._passive_stair_cooldown[env] = 0
        self._stair_event_buffer[env] = []

    def _record_stair_event(self, env: int, event_type: str, **payload: Any) -> None:
        self._stair_event_buffer[env].append({"event_type": event_type, **payload})

    def pop_stair_events(self, env: int) -> List[Dict[str, Any]]:
        events = list(self._stair_event_buffer[env])
        self._stair_event_buffer[env] = []
        return events
        ...
        if not self._reach_stair[env]:
            ...
            if self._stair_frontier[env] is not None and \
               np.linalg.norm(self._stair_frontier[env] - np.atleast_2d(robot_xy)) <= 0.3: # 注意这里原来是robot_xy_2d，改为robot_px[0]以匹配px_to_xy的转换
                ...
                self._stair_post_centroid_steps[env] = 0
                self._record_stair_event(
                    env,
                    "stair_reach_centroid",
                    direction="up" if climb_direction == 1 else "down",
                    floor_index=int(self._cur_floor_index[env]),
                )
        else: # _reach_stair_centroid[env] == True
            if already_reach_stair:
                self._stair_post_centroid_steps[env] += 1
                progress_px = self._compute_stair_progress_px(env, robot_px, climb_direction)
                min_progress_px = self.MIN_STAIR_PROGRESS_METERS * self._obstacle_map[env].pixels_per_meter
                if (
                    self._stair_post_centroid_steps[env] > self.STAIR_POST_CENTROID_LIMIT
                    and self._obstacle_map[env]._climb_stair_paused_step > 12
                    and progress_px < min_progress_px
                ) or (
                    self._stair_post_centroid_steps[env] > self.STAIR_POST_CENTROID_LIMIT * 2
                    and self._obstacle_map[env]._climb_stair_paused_step > 24
                ):
                    disabled_frontier = self._stair_frontier[env][0] if self._stair_frontier[env] is not None else np.array([])
                    self._disable_stair_and_reset_state(
                        env,
                        disabled_frontier,
            ...
            if not self.is_robot_in_stair_map_fast(env, robot_px, stair_map)[0] and \
               self._obstacle_map[env]._climb_stair_paused_step >= 30:
                ...
                stair_start = (
                    self._obstacle_map[env]._up_stair_start
                    if climb_direction == 1
                    else self._obstacle_map[env]._down_stair_start
                )
                progress_px = self._compute_stair_progress_px(env, robot_px, climb_direction)
                min_progress_px = self.MIN_STAIR_PROGRESS_METERS * self._obstacle_map[env].pixels_per_meter
                if progress_px < min_progress_px:
                    disabled_frontier = self._stair_frontier[env][0] if self._stair_frontier[env] is not None else np.array([])
                    self._disable_stair_and_reset_state(
                        env,
                        disabled_frontier,
                        reason="insufficient_stair_progress",
                    )
                    return
                self._reset_stair_climb_state(env)
                self._climb_stair_over[env] = True
                from_floor = int(self._cur_floor_index[env])
                if climb_direction == 1:
                ...
                self._record_stair_event(
                    ...
                    to_floor=int(self._cur_floor_index[env]),
                    progress_meters=progress_px / self._obstacle_map[env].pixels_per_meter if len(stair_start) > 0 else 0.0,
                )
    ...
    def _disable_stair_and_reset_state(self, env: int, disabled_frontier: np.ndarray, is_reverse: bool = False, reason: str = "stair_disabled"):
        """
        辅助函数：禁用楼梯并重置相关状态。
        将重复的楼梯禁用和状态重置逻辑封装起来。
        """
        original_climb_flag = self._climb_stair_flag[env]
        # 确保禁用前沿是可哈希的
        ...
        self._passive_up_stair_steps[env] = 0
        self._passive_down_stair_steps[env] = 0
        self._passive_stair_cooldown[env] = self.PASSIVE_STAIR_COOLDOWN_STEPS
        ...
        # 根据上楼/下楼情况更新地图和楼层信息
        if original_climb_flag == 1: # 上楼被禁用
        ...
        elif original_climb_flag == 2: # 下楼被禁用
        ...
        self._record_stair_event(
            ...
            "stair_disabled",
            reason=reason,
            direction="up" if original_climb_flag == 1 else "down" if original_climb_flag == 2 else "unknown",
            floor_index=int(self._cur_floor_index[env]),
            disabled_frontier=disabled_frontier.tolist() if disabled_frontier.size > 0 else [],
        )
    ...
    def _update_obstacle_map(self, observations_cache: List[Dict], red_semantic_pred_list: List[np.array], pitch_angle: List[int]) -> None:
        for env in range(self._num_envs):
            ...
            if self._passive_stair_cooldown[env] > 0:
                self._passive_stair_cooldown[env] -= 1
                ...
                if self._passive_up_stair_steps[env] >= self.PASSIVE_STAIR_DETECTION_THRESHOLD:
                    ...
                    if next_floor_idx >= len(self._obstacle_map_list[env]) or \
                       not self._obstacle_map_list[env][next_floor_idx]._this_floor_explored:
                        ...
                        self._record_stair_event(env, "passive_stair_trigger", direction="up", floor_index=int(self._cur_floor_index[env]))
                ...
                if self._passive_down_stair_steps[env] >= self.PASSIVE_STAIR_DETECTION_THRESHOLD:
                    ...
                    if prev_floor_idx < 0 or \
                       not self._obstacle_map_list[env][prev_floor_idx]._this_floor_explored:
                        ...
                        self._record_stair_event(env, "passive_stair_trigger", direction="down", floor_index=int(self._cur_floor_index[env]))
    ...
    def _compute_stair_progress_px(self, env: int, robot_px: np.ndarray, climb_direction: int) -> float:
        stair_start = (
            self._obstacle_map[env]._up_stair_start
            if climb_direction == 1
            else self._obstacle_map[env]._down_stair_start
        )
        if len(stair_start) == 0:
            return 0.0
        return float(np.linalg.norm(robot_px[0] - stair_start))
