...
from ascent.llm_config import LLMBackendConfig
...
def generate_video(
    ...
    # my preferred name
    video_name = (
        f"scene={_slugify_video_component(scene_id)}-"
        f"episode={_slugify_video_component(episode_id)}-"
        f"goal={_slugify_video_component(goal_name)}-"
        + "-".join(metric_strs)
        + "-"
        + _slugify_video_component(failure_cause)
    )
...
def _slugify_video_component(value: Union[int, str]) -> str:
    return (
        str(value)
        .strip()
        .replace("/", "_")
...
class MultiFloorTopDownMap(FrontierExplorationMap):
    ...
    def detect_floors(self, output_dir="./", sample_points=100, floor_tolerance=0.5):
        ...
        if len(floor_heights) == 0:
            try:
                floor_heights = [float(self._sim.get_agent_state().position[1])]
            except Exception:
                floor_heights = [0.0]
