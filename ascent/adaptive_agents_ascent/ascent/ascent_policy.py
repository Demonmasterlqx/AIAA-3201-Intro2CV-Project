...
class Ascent_Policy(HabitatMixin, ITMPolicyV2):
    ...
    def from_config(cls, config: DictConfig, *args_unused: Any, **kwargs_unused: Any) -> "Ascent_Policy":
        ...
        # 添加 Habitat 相关参数
        primary_agent_name = (
            config.habitat.simulator.agents_order[0]
            if len(config.habitat.simulator.agents_order) > 1
            else "main_agent"
        )
        sim_sensors_cfg = config.habitat.simulator.agents[primary_agent_name].sim_sensors
        ...
        kwargs["llm_config"] = policy_config.llm
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        pointnav_device = "cpu" if len(config.habitat.simulator.agents_order) > 1 else ("cuda" if torch.cuda.is_available() else "cpu")
        self._pointnav_policy: List[WrappedPointNavResNetPolicy] = [
            WrappedPointNavResNetPolicy(
                kwargs["pointnav_policy_path"],
                original_config=config,
                agent_name="agent_0" if len(config.habitat.simulator.agents_order) > 1 else "main_agent",
        ...
        if config:
            ...
            primary_height = config.habitat.simulator.agents[primary_agent_name].sim_sensors.rgb_sensor.height
            self._cx, self._cy = self._image_width // 2, primary_height // 2
        else:
            self._cx, self._cy = self._image_width // 2, self._image_height // 2
        ...
        if config:
            task_actions = config.habitat.task.actions
            if "look_down" in task_actions and hasattr(task_actions.look_down, "tilt_angle"):
                self._pitch_angle_offset = task_actions.look_down.tilt_angle
            elif "joint_nav_action" in task_actions and hasattr(task_actions.joint_nav_action, "tilt_angle"):
                self._pitch_angle_offset = task_actions.joint_nav_action.tilt_angle
            else:
                self._pitch_angle_offset = self.DEFAULT_PITCH_ANGLE_OFFSET
        else:
            self._pitch_angle_offset = self.DEFAULT_PITCH_ANGLE_OFFSET
        ...
        self._stair_nav_repeat_signature: List[str] = [""] * self._num_envs
        self._stair_nav_repeat_steps: List[int] = [0] * self._num_envs
        self._stair_nav_repeat_limit: int = 60
    ...
    def _reset(self, env: int) -> None:
        ...
        self._stair_nav_repeat_signature[env] = ""
        self._stair_nav_repeat_steps[env] = 0
    ...
    def _get_policy_info(self, detections: ObjectDetections,  env: int = 0) -> Dict[str, Any]: # seg_map_color:np.ndarray,
        ...
        policy_info.update(self.llm_planner.get_logging_info())
        ...
        **kwargs: Any,
        ...
        for env in range(self._num_envs):
            ...
            stair_locked_mode = (
                ...
                or self._map_controller._climb_stair_flag[env] in [1, 2]
                or mode in {
                    "get_close_to_stair",
                    "climb_stair",
                    "look_up",
            ...
            if len(self.history_action[env]) >= 30:
                self.history_action[env].pop(0)

            # Check history and potentially override the action
            if not stair_locked_mode and len(self.history_action[env]) >= 29: # window already trimmed above
    ...
    def _explore(self, observations: Union[Dict[str, Tensor], "TensorDict"], env: int, masks: Tensor) -> Tensor:
        ...
        if len(frontiers) > 0 and not np.array_equal(frontiers, np.zeros((1, 2))):
            self._stair_nav_repeat_signature[env] = ""
            self._stair_nav_repeat_steps[env] = 0
        ...
        if np.array_equal(frontiers, np.zeros((1, 2))) or len(frontiers) == 0:
            ...
            if action is not None:
                ...
                if not self._map_controller._obstacle_map[env]._reinitialize_flag:
                    print(
                        f"Environment {env}: no frontiers or usable stairs remain, "
                        "triggering one recovery reinitialization."
                    )
                    return self._handle_stairwell_reinitialization(env, masks)
    ...
    def _navigate_stair_if_unexplored_floor(self, observations: Union[Dict[str, Tensor], "TensorDict"], env: int, direction: str) -> Optional[Tensor]:
        ...
        if getattr(self._map_controller._obstacle_map[env], has_stair_attr):
            ...
            if temp_flag:
                stair_frontiers = getattr(self._map_controller._obstacle_map[env], stair_frontiers_attr)
                if stair_frontiers.size == 0:
                    self._stair_nav_repeat_signature[env] = ""
                    self._stair_nav_repeat_steps[env] = 0
                    return None
                target_stair_frontier = stair_frontiers[0]
                signature = (
                    f"{direction}|floor={self._map_controller._cur_floor_index[env]}|"
                    f"x={target_stair_frontier[0]:.3f}|y={target_stair_frontier[1]:.3f}"
                )
                if signature == self._stair_nav_repeat_signature[env]:
                    self._stair_nav_repeat_steps[env] += 1
                else:
                    self._stair_nav_repeat_signature[env] = signature
                    self._stair_nav_repeat_steps[env] = 1

                if self._stair_nav_repeat_steps[env] > self._stair_nav_repeat_limit:
                    print(
                        f"Environment {env}: stair navigation timed out for {signature}. "
                        "Disabling the stair frontier and falling back."
                    )
                    self._map_controller._climb_stair_flag[env] = climb_flag_value
                    self._map_controller._stair_frontier[env] = stair_frontiers
                    self._map_controller._disable_stair_and_reset_state(
                        env,
                        target_stair_frontier,
                        reason="stair_navigation_timeout",
                    )
                    self._stair_nav_repeat_signature[env] = ""
                    self._stair_nav_repeat_steps[env] = 0
                    return None
    ...
    def _pointnav(self, observations: "TensorDict", goal: np.ndarray, stop: bool = False, env: int = 0, ori_masks: Tensor = None, stop_radius: float = 0.9) -> Tensor: #, 
        ...
        """
        pointnav_device = self._pointnav_policy[env].device
        masks = torch.tensor([self._num_steps[env] != 0], dtype=torch.bool, device=pointnav_device)
        ...
        rho_theta_tensor = torch.tensor([[rho, theta]], device=pointnav_device, dtype=torch.float32)
        obs_pointnav = {
            "depth": image_resize(
    ...
    def _climb_stair(self, observations: "TensorDict", env: int, ori_masks: Tensor) -> Tensor:
        ...
        if (
            ...
            and self._map_controller._obstacle_map[env]._climb_stair_paused_step > 24
        ):
            print(
                f"Climb stair stalled inside stair area. Disabling stair frontier {target_stair_frontier[0]}."
            )
            self._map_controller._disable_stair_and_reset_state(
                env,
                target_stair_frontier[0],
                reason="stair_no_exit_progress",
            )
            return self._explore(observations, env, ori_masks)
