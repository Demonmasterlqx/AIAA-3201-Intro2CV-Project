...
def extract_scalars_from_info(info: Dict[str, Any]) -> Dict[str, float]:
    ...
    return _sanitize_metrics(extract_scalars_from_info_habitat(info_filtered))


def _sanitize_metric_value(key: str, value: float) -> float:
    if np.isfinite(value):
        return float(value)
    if key == "distance_to_goal":
        return -1.0
    if key.endswith("reward"):
        return 0.0
    if key in {"spl", "soft_spl"}:
        return 0.0
    return 0.0
...
def _sanitize_metrics(metrics: Dict[str, Any]) -> Dict[str, float]:
    ...
    for key, value in metrics.items():
        try:
            ...
            continue
        sanitized[key] = _sanitize_metric_value(key, numeric)
    return sanitized


def summarize_policy_info(policy_info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key, value in policy_info.items():
        if isinstance(value, (int, float, str, bool)):
            ...
            if len(value) <= 8 and all(isinstance(item, (int, float, str, bool)) for item in value):
                summary[key] = value
    return summary


def _materialize_episode_video(video_path: str, episode_log_dir: str) -> str:
    if not video_path or not episode_log_dir:
        return video_path
    src_path = Path(video_path)
    if not src_path.exists():
        return video_path
    dst_path = Path(episode_log_dir) / "video.mp4"
...
class AscentTrainer(PPOTrainer):
    ...
    def _eval_checkpoint(
        ...
        with read_write(config):
            ...
            if "results/as_ascent" in str(config.habitat_baselines.video_dir):
                run_suffix = datetime.now().strftime("%Y%m%d_%H%M%S")
                config.habitat_baselines.video_dir = str(Path(config.habitat_baselines.video_dir) / run_suffix)
            if "results/as_ascent" in str(config.habitat_baselines.tensorboard_dir):
                run_suffix = datetime.now().strftime("%Y%m%d_%H%M%S")
                config.habitat_baselines.tensorboard_dir = str(Path(config.habitat_baselines.tensorboard_dir) / run_suffix)
        ...
        while len(stats_episodes) < (number_of_eval_episodes * evals_per_ep) and self.envs.num_envs > 0:
            ...
            for i in range(n_envs):
                ...
                if len(self.config.habitat_baselines.eval.video_option) > 0:
                    action_data.policy_info[i]["scene_id"] = current_episodes_info[i].scene_id.split("/")[-1].split(".")[0]
                    action_data.policy_info[i]["episode_id"] = str(current_episodes_info[i].episode_id)
                ...
                if not not_done_masks[i].item():
                    pbar.update()
                    reward_value = float(current_episode_reward[i].item())
                    if not np.isfinite(reward_value):
                        reward_value = 0.0
                    episode_stats = {"reward": reward_value}
                    ...
                    if len(self.config.habitat_baselines.eval.video_option) > 0:
                        ...
                        if video_name:
                            video_path = str(Path(self.config.habitat_baselines.video_dir) / f"{video_name}.mp4")

                        rgb_frames[i] = []

                    episode_log_dir = action_data.policy_info[i].get("episode_log_dir", "")
                    if episode_log_dir:
                        episode_video_path = _materialize_episode_video(video_path, episode_log_dir)
                        episode_logger = ASEpisodeLogger(episode_log_dir)
                        policy_summary = summarize_policy_info(action_data.policy_info[i])
                        resolved_goal_name = (
                            goal_name[i]
                            or str(action_data.policy_info[i].get("target_object", "")).strip()
                            or str(getattr(current_episodes_info[i], "object_category", "")).strip()
                        )
                        summary_payload = {
                            "scene_id": f"{current_episodes_info[i].scene_id}".split("/")[-1].split(".")[0],
                            "episode_id": current_episodes_info[i].episode_id,
                            ...
                            **metrics,
                            ...
                            "llm_backend": action_data.policy_info[i].get("llm_backend", ""),
                            "llm_model": action_data.policy_info[i].get("llm_model", ""),
                            "spawn_count": action_data.policy_info[i].get("spawn_count", 0),
                            "reclaim_count": action_data.policy_info[i].get("reclaim_count", 0),
                            "subagent_state_final": action_data.policy_info[i].get("subagent_state", ""),
                            "coordinator_state_final": action_data.policy_info[i].get("coordinator_state", ""),
                            "video_layout_version": action_data.policy_info[i].get("video_layout_version", ""),
                            "video_path": episode_video_path or video_path,
                            "rendered_video_path": video_path,
                        }
                        episode_logger.log_summary(summary_payload)
