...
RUN_ID="deepseek_p53_ep0_forced_upstairs_video_$(date +%Y%m%d_%H%M%S)"
...
python "${ROOT}/make_p53_ep0_split.py" | tee "${RESULT_DIR}/split_creation.txt"
...

{
  ...
  echo "split=val_p53_ep0"
  echo "scene=p53SfW6mjZe"
  echo "forced_episode=p53SfW6mjZe original content index 0, goal sofa/couch"
  echo "expected_old_log=upstairs success, SPL about 0.798, DTG about 0.017, steps about 72/73"
  echo
} | tee "${RESULT_DIR}/run_metadata.txt"

python -u -m ascent.run \
  --config-name=eval_ascent_hm3d.yaml \
  habitat_baselines.eval.split=val_p53_ep0 \
  habitat.dataset.split=val_p53_ep0 \
