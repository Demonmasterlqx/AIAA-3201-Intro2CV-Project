...
class WrappedPointNavResNetPolicy:
    ...
    def __init__(
        ...
        self.policy = load_pointnav_policy(
            ckpt_path,
...
def load_pointnav_policy(
    file_path: str,
    original_config=None,
    agent_name: str = "main_agent",
) -> PointNavResNetTensorOutputPolicy:
    """Loads a PointNavResNetPolicy policy from a .pth file. Basaed on VLFM's policy, but add more robustness.

    Args:
        file_path (str): The path to the trained weights of the pointnav policy.
        original_config: The original config (needed for multi-agent setup)
    Returns:
    ...
    if HABITAT_BASELINES_AVAILABLE:
        ...
        if habitat_version == "0.1.5":
            ...
            if original_config is not None:
                config_to_use = original_config
            else:
                config_to_use = ckpt_dict["config"]
            if (
                original_config is not None
                and hasattr(config_to_use.habitat_baselines.rl, "policy")
                and agent_name not in config_to_use.habitat_baselines.rl.policy
                and "main_agent" in config_to_use.habitat_baselines.rl.policy
            ):
                config_to_use = OmegaConf.create(
                    OmegaConf.to_container(config_to_use, resolve=True)
                )
                config_to_use.habitat_baselines.rl.policy[agent_name] = config_to_use.habitat_baselines.rl.policy["main_agent"]
...
def move_obs_to_device(
    ...
    for k, v in observations.items():
        if isinstance(v, np.ndarray):
            ...
            observations[k] = v.to(device=device)
            if unsqueeze:
                observations[k] = observations[k].unsqueeze(0)
