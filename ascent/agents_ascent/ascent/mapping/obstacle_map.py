...
class ObstacleMap(BaseMap):
    ...
    def extract_frontiers_with_image(self, frontier):
        ...
        """
        frontier_key = tuple(frontier)
        if frontier_key in self.frontier_visualization_info:
            floor_num_steps = self.frontier_visualization_info[frontier_key]['floor_num_steps']
            visualized_rgb = self._each_step_rgb[floor_num_steps].copy()
            return floor_num_steps, visualized_rgb

        if self.frontier_visualization_info:
            nearest_key = min(
                self.frontier_visualization_info.keys(),
                key=lambda key: np.linalg.norm(np.array(key) - np.array(frontier_key)),
            )
            floor_num_steps = self.frontier_visualization_info[nearest_key]['floor_num_steps']
            visualized_rgb = self._each_step_rgb[floor_num_steps].copy()
            return floor_num_steps, visualized_rgb

        if self._each_step_rgb:
            floor_num_steps = max(self._each_step_rgb.keys())
            visualized_rgb = self._each_step_rgb[floor_num_steps].copy()
            return floor_num_steps, visualized_rgb

        raise KeyError(f"No stored RGB frame is available for frontier {frontier_key}")
