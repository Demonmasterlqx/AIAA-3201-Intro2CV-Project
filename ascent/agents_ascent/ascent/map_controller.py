...
class Map_Controller:
    ...
    def __init__(self, device, num_envs: int, map_size: int = 1600, text_prompt: str = "", object_map_erosion_size: int = 1600,
        ...
        self._target_confirm_streak = int(os.environ.get("AS_TARGET_CONFIRM_STREAK", "2"))
        self._target_confirm_high_conf = float(os.environ.get("AS_TARGET_CONFIRM_HIGH_CONF", "0.55"))
        self._target_confirm_center_tolerance_px = float(
            os.environ.get("AS_TARGET_CONFIRM_CENTER_TOLERANCE_PX", "80.0")
        )
        self._target_blip_confirm_threshold = float(
            os.environ.get("AS_TARGET_BLIP_CONFIRM_THRESHOLD", "0.19")
        )
        ...
        self._target_detection_streak: List[int] = [0] * self._num_envs
        self._target_last_center: List[Optional[np.ndarray]] = [None] * self._num_envs
    ...
    def reset(self, env: int) -> None:
        ...
        self._target_detection_streak[env] = 0
        self._target_last_center[env] = None
    ...
    def _update_object_map_with_stair_and_person(self, height: int, width: int, observations_cache: List[Dict], non_coco_caption: str, num_steps: List[int], try_to_navigate: List[bool]):
        ...
        for env in range(self._num_envs):
            ...
            target_detections = self._stabilize_target_detections(
                env,
                target_detections,
                width,
                height,
            ...
            for idx in range(len(target_detections.logits)):
                ...
                if try_to_navigate[env] and not self._double_check_goal[env]:
                    ...
                    if match_score >= self._target_blip_confirm_threshold:
    ...
    def _stabilize_target_detections(
        self,
        env: int,
        ...
        if detections is None or getattr(detections, "num_detections", 0) <= 0:
            self._target_detection_streak[env] = 0
            self._target_last_center[env] = None
            return detections
        ...
        best_box = detections.boxes[best_idx]
        ...
        center = np.array(
            [
                float((best_box[0] + best_box[2]) * 0.5 * width),
                float((best_box[1] + best_box[3]) * 0.5 * height),
            ],
            dtype=np.float32,
        )
        last_center = self._target_last_center[env]
        if last_center is not None and float(np.linalg.norm(center - last_center)) <= self._target_confirm_center_tolerance_px:
            self._target_detection_streak[env] += 1
        else:
            self._target_detection_streak[env] = 1
        self._target_last_center[env] = center

        if (
            best_score >= self._target_confirm_high_conf
            or self._target_detection_streak[env] >= self._target_confirm_streak
        ):
            return detections

        gated = deepcopy(detections)
        gated.filter_by_conf(2.0)
        return gated
