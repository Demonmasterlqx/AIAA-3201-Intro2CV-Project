...
class Ascent_Policy(HabitatMixin, ITMPolicyV2):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if torch.cuda.is_available():
            ...
            pointnav_device = f"cuda:{cuda_id}"
        else:
            pointnav_device = "cpu"
    ...
    def _navigate(self, observations: "TensorDict", goal: np.ndarray, stop: bool = False, env: int = 0, ori_masks: Tensor = None, stop_radius: float = 0.9) -> Tensor:
        ...
        if self._map_controller.cur_dis_to_goal[env] < 1.0: # close to the goal, but might be some noise, so get close as possible 
            close_enough = self._map_controller.cur_dis_to_goal[env] <= 0.45
            stalled_near_goal = (
                self._map_controller.cur_dis_to_goal[env] <= 0.55
                and np.abs(self._map_controller.cur_dis_to_goal[env] - self.min_distance_xy[env]) < 0.05
            )
            if close_enough or stalled_near_goal: # close enough or cannot move forward more #  or self._num_steps[env] == (500 - 1)
