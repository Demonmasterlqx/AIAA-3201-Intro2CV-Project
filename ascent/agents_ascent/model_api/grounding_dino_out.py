...
# try:
try:
    from third_party.GroundingDINO.groundingdino.util.inference import load_model, predict
except ModuleNotFoundError:
    load_model = None
    predict = None
    print("Could not import groundingdino. This is OK if you are only using the client.")

GROUNDING_DINO_CONFIG = "third_party/GroundingDINO/groundingdino/config/GroundingDINO_SwinT_OGC.py"
GROUNDING_DINO_WEIGHTS = "pretrained_weights/groundingdino_swint_ogc.pth"
CLASSES = "chair . person . dog ."  # Default classes. Can be overridden at inference.
import os
# os.environ["OUT_HOST"] = 


def _empty_detections(image: Optional[np.ndarray]) -> ObjectDetections:
...
class GroundingDINO:
    def __init__(
        ...
        if load_model is None:
            raise ModuleNotFoundError(
                "GroundingDINO is unavailable. Use GroundingDINOClient or initialize third_party/GroundingDINO."
            )
    ...
    def predict(self, image: np.ndarray, caption: Optional[str] = None) -> ObjectDetections:
        ...
        if image is None or image.size == 0:
            print("GroundingDINO received an empty image; returning no detections.")
            return _empty_detections(image)
        if image.ndim == 2:
        ...
        try:
            ...
            if "selected index k out of range" not in str(exc):
                raise
            print(
                "GroundingDINO fallback: returning no detections "
                f"for caption={caption_to_use!r}, image_shape={image.shape}: {exc}"
            )
            return _empty_detections(image)
...
if __name__ == "__main__":
    ...
    class GroundingDINOServer(ServerMixin, GroundingDINO):
        def process_payload(self, payload: dict) -> dict:
            # 处理 GET 请求
            if payload is None:  
                return {"status": "ok", "message": "Grounding Dino service is running"}
