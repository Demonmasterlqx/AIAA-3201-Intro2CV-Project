...
d_fine_path = "third_party/D-FINE/"
try:
    sys.path.insert(0, d_fine_path)
    from src.core import YAMLConfig
except ModuleNotFoundError:
    YAMLConfig = None
    print("Could not import dfine. This is OK if you are only using the client.")
finally:
    if sys.path and sys.path[0] == d_fine_path:
        sys.path.pop(0)
    ...
    def __init__(self, config_path: str, checkpoint_path: str, device: str = "cpu"):
        ...
        if YAMLConfig is None:
            raise ModuleNotFoundError(
                "D-FINE is unavailable. Use DFineClient or initialize third_party/D-FINE."
            )
