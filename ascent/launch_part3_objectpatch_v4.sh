...
DST_RESULT="$OBJ_REPO/results/baseline_compare_single/qwenvl_objectpatch_weakcache_1000eps_rerun_v4"
QUEUE_LOG="$BASE/part3_1000eps_objectpatch_v4.log"
SESSION="part3_objectpatch_1000eps_v4"

mkdir -p "$DST_RESULT"
cp "$SRC_RESULT/run_command.sh" "$DST_RESULT/run_command.sh"
chmod +x "$DST_RESULT/run_command.sh"

if [ ! -e "$OBJ_REPO/data" ]; then
  ln -s "$BASE/ascent/data" "$OBJ_REPO/data"
fi
test -f "$OBJ_REPO/data/datasets/objectnav/hm3d/v1/val/val.json.gz"
...
cat > "$BASE/part3_objectpatch_1000eps_v4_inner.sh" <<'INNER'
#!/usr/bin/env bash
set -u pipefail
source /data/home/sim6g/anaconda3/etc/profile.d/conda.sh
conda activate ascent_nav
...
DST_RESULT="$OBJ_REPO/results/baseline_compare_single/qwenvl_objectpatch_weakcache_1000eps_rerun_v4"
QUEUE_LOG="$BASE/part3_1000eps_objectpatch_v4.log"
printf 'PART3 objectpatch v4 started at %s\n' "$(date -Is)" | tee "$QUEUE_LOG"
printf 'OBJ_REPO=%s\nDST_RESULT=%s\n' "$OBJ_REPO" "$DST_RESULT" | tee -a "$QUEUE_LOG"
printf 'DATA_LINK=%s\n' "$(readlink -f "$OBJ_REPO/data")" | tee -a "$QUEUE_LOG"
cd "$OBJ_REPO"
bash "$DST_RESULT/run_command.sh" > "$DST_RESULT/run.log" 2>&1
code=$?
echo "EXIT_CODE:$code" >> "$DST_RESULT/run.log"
printf 'PART3 objectpatch v4 finished with %s at %s\n' "$code" "$(date -Is)" | tee -a "$QUEUE_LOG"
exit "$code"
INNER
chmod +x "$BASE/part3_objectpatch_1000eps_v4_inner.sh"

tmux new-session -d -s "$SESSION" "bash '$BASE/part3_objectpatch_1000eps_v4_inner.sh'"
printf 'launched %s\n' "$SESSION"
