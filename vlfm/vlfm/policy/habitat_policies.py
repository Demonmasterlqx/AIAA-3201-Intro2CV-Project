...
from .itm_policy import (
    AdaptiveHybridITMPolicy,
    CollaborativeAdaptiveHybridITMPolicy,
    DenseValueMatchingPolicy,
    ITMPolicy,
    ITMPolicyV2,
    ITMPolicyV3,
)
...
class HabitatMixin:
    ...
    def from_config(cls, config: DictConfig, *args_unused: Any, **kwargs_unused: Any) -> "HabitatMixin":
        ...
        dataset_hints = [
            str(config.habitat.dataset.data_path),
            str(getattr(config.habitat.dataset, "scenes_dir", "")),
            str(getattr(config.habitat.simulator, "scene_dataset", "")),
        ]
        if any("hm3d" in hint for hint in dataset_hints):
            kwargs["dataset_type"] = "hm3d"
        elif any("mp3d" in hint for hint in dataset_hints):
...
@baseline_registry.register_policy
class HabitatDenseValueMatchingPolicy(HabitatMixin, DenseValueMatchingPolicy):
    pass


@baseline_registry.register_policy
class HabitatAdaptiveHybridPolicy(HabitatMixin, AdaptiveHybridITMPolicy):
    pass


@baseline_registry.register_policy
class HabitatCollaborativeAdaptiveHybridPolicy(HabitatMixin, CollaborativeAdaptiveHybridITMPolicy):
    pass
