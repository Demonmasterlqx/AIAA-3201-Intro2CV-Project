...
class ValueMap(BaseMap):
    ...
    def get_value_peaks(
        ...
        """Return local maxima from the dense value map sorted by score.

        This is useful for baselines that navigate directly to high-value regions in
        the map instead of first restricting the search space to frontier points.

        Args:
            radius: Suppression radius in meters. Nearby maxima are merged into a single
                candidate.
            reduce_fn: Optional reduction for multi-channel value maps. When omitted,
                multi-channel maps are reduced with ``np.max``.
            obstacle_map: Optional obstacle map used to mask the candidates. Defaults to
                the obstacle map attached to the value map, if available.
            min_value: Minimum value threshold for a candidate to be considered.
            max_points: Optional limit on the number of returned points.

        Returns:
            Sorted candidate points in episodic ``(x, y)`` coordinates and their
            corresponding values.
        """
        reduced_map = self._reduce_value_map(reduce_fn)
        map_obstacle = obstacle_map if obstacle_map is not None else self._obstacle_map

        valid_mask = np.isfinite(reduced_map)
        if map_obstacle is not None:
            valid_mask &= map_obstacle.explored_area.astype(bool)

        if not np.any(valid_mask):
            return np.array([]), []

        scores = reduced_map.astype(np.float32).copy()
        fill_value = np.finfo(np.float32).min
        scores[~valid_mask] = fill_value

        radius_px = max(1, int(radius * self.pixels_per_meter))
        kernel = cv2.getStructuringElement(
            cv2.MORPH_ELLIPSE,
            (2 * radius_px + 1, 2 * radius_px + 1),
        )
        dilated_scores = cv2.dilate(scores, kernel)
        eroded_scores = cv2.erode(scores, kernel)
        peak_mask = valid_mask & (scores >= min_value) & (scores == dilated_scores) & (scores > eroded_scores)
        ...
        if np.any(peak_mask):
            ...
            for label_idx in range(1, num_labels):
                ...
                values = scores[labels == label_idx]
                best_idx = int(np.argmax(values))
                peak_coords.append(coords[best_idx])
                peak_values.append(float(values[best_idx]))
            peak_coords_arr = np.array(peak_coords, dtype=int)
        else:
            best_row, best_col = np.unravel_index(np.argmax(scores), scores.shape)
            if scores[best_row, best_col] == fill_value:
                return np.array([]), []
            peak_coords_arr = np.array([[best_row, best_col]], dtype=int)
            peak_values = [float(scores[best_row, best_col])]

        order = np.argsort(peak_values)[::-1]
        if max_points is not None:
            order = order[:max_points]

        peak_coords_arr = peak_coords_arr[order]
        sorted_values = [peak_values[i] for i in order]
        peak_pixels = np.stack((peak_coords_arr[:, 1], peak_coords_arr[:, 0]), axis=1)

        return self._px_to_xy(peak_pixels), sorted_values

    def _reduce_value_map(self, reduce_fn: Optional[Callable] = None) -> np.ndarray:
        if self._value_channels == 1:
            return self._value_map[..., 0].copy()

        if reduce_fn is None:
            return np.max(self._value_map, axis=-1)

        return reduce_fn(self._value_map)
