...
from vlfm.utils.eval_artifacts import (
    DEFAULT_VIDEO_VIEWS,
    get_episode_dir,
    parse_video_views,
    save_episode_videos,
...
from vlfm.utils.episode_stats_logger import build_episode_stats
from vlfm.utils.episode_stats_logger import log_episode_stats
from vlfm.utils.habitat_visualizer import HabitatVis, grid_xy_to_sim_xy
...
def _get_episode_root() -> Optional[Path]:
    root = os.environ.get("VLFM_EPISODE_ROOT_DIR", "").strip()
    return Path(root) if root else None


def _get_requested_video_views() -> List[str]:
    raw = os.environ.get("VLFM_VIDEO_VIEWS", ",".join(DEFAULT_VIDEO_VIEWS))
    return parse_video_views(raw)


def _extract_agent_position_from_info(info: Dict[str, Any]) -> Optional[np.ndarray]:
    top_down_map = info.get("top_down_map")
    if top_down_map is None or "agent_map_coord" not in top_down_map:
        return None

    grid_xy = np.asarray(top_down_map["agent_map_coord"], dtype=np.float32).reshape(1, 2)
    sim_xy = grid_xy_to_sim_xy(
        top_down_map["upper_bound"],
        top_down_map["lower_bound"],
        top_down_map["grid_resolution"],
        grid_xy,
    )
    return sim_xy[0]
...
class VLFMTrainer(PPOTrainer):
    ...
    def _eval_checkpoint(
        ...
        num_total = 0
        hab_vis = HabitatVis()
        episode_root = _get_episode_root()
        requested_video_views = _get_requested_video_views()
        episode_step_counts = [0 for _ in range(self.envs.num_envs)]
        episode_path_lengths = [0.0 for _ in range(self.envs.num_envs)]
        episode_inference_times = [0.0 for _ in range(self.envs.num_envs)]
        episode_start_times = [perf_counter() for _ in range(self.envs.num_envs)]
        episode_last_positions: List[Optional[np.ndarray]] = [None for _ in range(self.envs.num_envs)]
        ...
        while len(stats_episodes) < (number_of_eval_episodes * evals_per_ep) and self.envs.num_envs > 0:
            ...
            for i in range(n_envs):
                ...
                episode_inference_times[i] += inference_duration
                curr_position = _extract_agent_position_from_info(infos[i])
                if episode_last_positions[i] is not None and curr_position is not None:
                    episode_path_lengths[i] += float(np.linalg.norm(curr_position - episode_last_positions[i]))
                episode_last_positions[i] = curr_position
                if (
                    ep_eval_count[
                        (
                ...
                if not not_done_masks[i].item():
                    ...
                    episode_wall_time_sec = perf_counter() - episode_start_times[i]
                    avg_inference_time_sec = episode_inference_times[i] / max(episode_step_counts[i], 1)
                    episode_stats.update(
                        {
                            "step_count": float(episode_step_counts[i]),
                            "path_length_m": float(episode_path_lengths[i]),
                            "episode_wall_time_sec": float(episode_wall_time_sec),
                            "avg_inference_time_sec": float(avg_inference_time_sec),
                        }
                    )
                    ...
                    try:
                        if episode_root is None:
                            ...
                            failure_cause = build_episode_stats(
                    ...
                    if len(self.config.habitat_baselines.eval.video_option) > 0:
                        frame_sets = hab_vis.flush_frame_sets(failure_cause)
                        rgb_frames[i] = frame_sets.get("composite", [])
                        if episode_root is None:
                            generate_video(
                    ...
                    if episode_root is not None:
                        episode_id = str(current_episodes_info[i].episode_id)
                        scene_id_full = current_episodes_info[i].scene_id
                        scene_short = scene_id_to_short(scene_id_full)
                        episode_dir = get_episode_dir(episode_root, scene_short, episode_id)
                        stuck_low_progress = (
                            infos[i].get("success", 0.0) == 0.0
                            and episode_step_counts[i] >= max(40, max_episode_steps // 5)
                            and episode_path_lengths[i] < 1.0
                        )
                        video_paths = save_episode_videos(
                            episode_dir,
                            frame_sets,
                            fps=self.config.habitat_baselines.video_fps,
                            requested_views=requested_video_views,
                            run_root=episode_root.parent,
                        )
                        extra_data = {
                            "dataset": os.environ.get("VLFM_DATASET_NAME", ""),
                            "split": config.habitat_baselines.eval.split,
                            "scene_id_full": scene_id_full,
                            "scene_short": scene_short,
                            "seed": int(os.environ.get("VLFM_RUN_SEED", config.habitat.seed)),
                            "policy_name": config.habitat_baselines.rl.policy.name,
                            "step_count": episode_step_counts[i],
                            "path_length_m": episode_path_lengths[i],
                            "episode_wall_time_sec": episode_wall_time_sec,
                            "avg_inference_time_sec": avg_inference_time_sec,
                            "termination_hint": infos[i].get("termination_hint", ""),
                            "stuck_low_progress": stuck_low_progress,
                            "video_paths": video_paths,
                        }
                        structured_stats = build_episode_stats(
                            episode_id,
                            scene_id_full,
                            infos[i],
                            extra_data=extra_data,
                        )
                        write_json(episode_dir / "episode_stats.json", structured_stats)
                    ...
                    episode_path_lengths[i] = 0.0
                    episode_inference_times[i] = 0.0
                    episode_start_times[i] = perf_counter()
                    episode_last_positions[i] = None
